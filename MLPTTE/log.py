...
class logger_tb(object):
    def __init__(self, log_dir, description, code_backup, time_stamp=True):
        description = description.split(" ")
        if time_stamp == True:
            t_stamp = time.strftime("%Y-%m-%d-%H-%M-%S", time.localtime())
            description.append(t_stamp)
        ...
        self.g_step = 0
        if code_backup:
            backup_code(os.path.join(self.log_dir, 'code_snapshot.zip'), '.', ['.py', '.json', '.sh', '.txt', 'json'], ['result', '.vscode', 'data-info', '.idea', 'logs'])
