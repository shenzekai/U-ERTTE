...
class Meta(nn.Module):
    ...
    def forward(self, *args):
        ...
        pr_loss = 0
        er_loss = 0
        y, mid_y = self.net(departure, driver_id, weekday, start_id, end_id, all_real, all_flow, all_linkdistance, all_link_feature, mask, all_mid_num=all_mid_num)
        # 1. run the i-th task and compute loss for k=0
        if loss_func == mape:
            pr_loss = loss_func(mid_y, label_spt[1].float())
        else:
            pr_loss = loss_func(mid_y, label_spt[1].float())
            pr_MPIW = torch.mean(mid_y[:, 2] - mid_y[:, 0])
            # loss = loss_func(y, label_spt[0].float())
        # 训练模型后执行此代码
        all_loss += pr_loss
        all_loss += 0.1 * pr_MPIW
        # all_loss += loss
